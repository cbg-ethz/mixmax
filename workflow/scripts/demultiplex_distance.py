...
class demoTape:
    def __init__(self, in_file, cl_no, distance='manhattan'):
    ...
    def plot_heatmap(self, out_file='', cluster=True, rel_SNPs_only=False):
        ...
        cm.ax_row_colors.set_xlabel(
            'Clusters', fontsize=10, rotation=90, ha='right', va='top'
        )
        ...
        cm.ax_heatmap.set_xticklabels(
            cm.ax_heatmap.get_xticklabels(), fontsize=5, ha='center', va='top'
...
class demoTape_reads(demoTape):
    ...
    def __str__(self):
        return str(super()) + 'Distance metric: reads'
    ...
    def get_relevant_SNPS(self):
        ...
        q_vals_sym = [
            kstest(VAF_z[:, i], -1 * VAF_z[:, i]).pvalue * self.SNPs.size
        ...
        for chrom, chrom_SNPs in self.SNPs[rel_SNPs].groupby('CHR'):
            ...
            for SNP2_chrom_id in same_read_SNP:
                ...
                VAF_dist = euclidean(
                    self.VAF[:, SNP1][valid], self.VAF[:, SNP2][valid]
    ...
    def get_data(self, df):
        ...
        self.VAF = np.clip(np.where(self.dp > 0, VAF, np.nan), EPSILON, 1 - EPSILON)
        self.RAF = 1 - self.VAF
    ...
    def get_pairwise_dists(self):
        ...
        for i in np.arange(self.cells.size - 1):
            valid = (self.dp[i, self.rel_SNPs] > 0) & (
                self.dp[i + 1 :, self.rel_SNPs] > 0
            )
            dp_total = self.dp[i, self.rel_SNPs] + self.dp[i + 1 :, self.rel_SNPs]
            p12 = np.clip(
                (
                    self.alt[i, self.rel_SNPs]
                    + self.alt[i + 1 :, self.rel_SNPs]
                    + EPSILON
                )
                / (dp_total + EPSILON),
                EPSILON,
            ...
            logl = (
                self.alt[i, self.rel_SNPs] * np.log(self.VAF[i, self.rel_SNPs] / p12)
                + self.ref[i, self.rel_SNPs]
                * np.log(self.RAF[i, self.rel_SNPs] / p12_inv)
                + self.alt[i + 1 :, self.rel_SNPs]
                * np.log(self.VAF[i + 1 :, self.rel_SNPs] / p12)
                + self.ref[i + 1 :, self.rel_SNPs]
                * np.log(self.RAF[i + 1 :, self.rel_SNPs] / p12_inv)
            )

            norm = (
                self.norm_const[dp_total]
                - self.norm_const[self.dp[i, self.rel_SNPs]]
                - self.norm_const[self.dp[i + 1 :, self.rel_SNPs]]
            )
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default='reads',
        choices=['reads', 'manhattan', 'euclidean'],
